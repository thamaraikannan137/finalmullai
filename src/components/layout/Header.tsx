  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[60] border-b border-white/10 bg-dark/90 backdrop-blur-[18px] backdrop-saturate-[160%]">
      <div className="container-site grid h-[66px] grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-[14px] px-[22px]">
        <Link
          href="/"
          className="flex min-w-0 items-center gap-[11px] justify-self-start whitespace-nowrap text-white no-underline"
        ...
        <nav
          className="mq-desknav min-w-0 items-center gap-px justify-self-center rounded-xl border border-white/10 bg-white/5 p-[3px]"
          aria-label="Main"
        >
        ...
        <div className="flex shrink-0 items-center justify-self-end gap-2.5">
          <LangSwitch
            className={cn("hidden min-[1181px]:inline-flex", langSwitchClass)}
          />
          <a
          ...
          <button
            type="button"
            className="mq-burger hidden size-[42px] items-center justify-center rounded-[11px] border border-white/15 bg-white/5 p-0 text-xl leading-none text-white"
            aria-label="Menu"
            aria-expanded={open}
      ...
      {open ? (
        <div className="mq-mobilemenu border-t border-white/10 bg-dark/98 px-[18px] pt-3 pb-5 backdrop-blur-[18px]">
          <div className="mb-3 sm:hidden">
            <LangSwitch className={langSwitchClass} />
          </div>
            ...
            {content.nav.map((item) =>
              item.type === "page" ? (
                <Link
                  key={item.id}
                  href={navHref(item)}
                  className="border-b border-white/10 px-2.5 py-3.5 text-base font-medium text-white/90 no-underline"
                  onClick={() => setOpen(false)}
                >
                  {item.label}
                </Link>
